...
## Использование
* Главная страница: отображает общую статистику по рассылкам.
* Рассылки: можно создавать, редактировать и удалять.
* Сообщения: добавляется в конкретную рассылку по одному.
* Получатели: создаются и добавляются по несколько для каждой рассылки.
